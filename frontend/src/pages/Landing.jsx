    ...
    <div className="min-h-screen bg-brand-cream text-foreground">
      ...
      <header className="sticky top-0 z-30 backdrop-blur-xl bg-brand-cream/80 border-b border-brand-border">
        <div className="max-w-7xl mx-auto flex items-center justify-between px-4 sm:px-6 py-4">
          ...
          <nav className="flex items-center gap-2 sm:gap-3">
            <Link data-testid="landing-login-link" to="/login" className="pill-btn btn-outline text-sm">Log in</Link>
            <Link data-testid="landing-signup-link" to="/signup" className="pill-btn btn-primary text-sm">Start free</Link>
          </nav>
        </div>
      </header>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 pt-10 pb-14 lg:pt-20 lg:pb-24 grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
        <div className="lg:col-span-7 flex flex-col gap-6">
          ...
          <p className="text-lg text-muted-foreground max-w-xl">
            TiffinOS is the operating system for independent tiffin providers. Manage customers, delivery lists, Interac payments and outstanding balances — all from your phone.
          </p>
          <div className="flex flex-wrap gap-3 items-center">
            <Link data-testid="hero-cta-signup" to="/signup" className="pill-btn btn-primary h-12 px-6 text-base gap-2">
              Start free <ArrowRight size={18} weight="bold" />
            </Link>
            <Link data-testid="hero-cta-consumer" to="/consumer-signup" className="pill-btn btn-outline h-12 px-6 text-base">
              I'm a consumer
            </Link>
          </div>
        ...
        <div className="lg:col-span-5 relative">
          <div className="relative rounded-3xl overflow-hidden border border-brand-border shadow-xl">
            <img src={HERO_IMG} alt="Partitioned Indian lunchbox" className="w-full h-[420px] object-cover" />
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 py-14 lg:py-20 grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="card-tinted p-8 flex flex-col gap-5">
          ...
          <Link to="/signup" data-testid="cta-provider" className="pill-btn btn-primary self-start">Start a workspace</Link>
        </div>
        <div className="card-tinted p-8 flex flex-col gap-5">
          ...
          <Link to="/consumer-signup" data-testid="cta-consumer" className="pill-btn btn-secondary self-start">Join with a code</Link>
        </div>
      </section>
      ...
      <footer className="border-t border-brand-border py-10 mt-8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div className="font-display font-bold text-lg">TiffinOS<span className="text-primary">.</span></div>
