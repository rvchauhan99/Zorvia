...
export default function CustomerFormPage({
  ...
  useEffect(() => {
    if (mode !== "edit") {
      setForm({
        ...
        meal_type_id: "regular",
        meal_price: String(mealTypes.find((t) => t.id === "regular")?.price ?? 12),
      });
      return;
    }
    const c = initialData;
  ...
  function toggleDay(i: number) {
    setForm((f) => {
      ...
      if (on) delete schedule[String(i)];
      else schedule[String(i)] = scheduleMode === "same" ? f.meal_quantity : 1;
            ...
            <div className={`grid grid-cols-1 sm:grid-cols-2 gap-3`}>
              {canMutate ? (
                <>
                  ...
                  <FieldLabel
                    label="Price per meal (CAD)"
                    hint="Leave blank to use meal type default"
                  >
                    <input
                      type="number"
                      step="0.5"
                      ...
                      className={inp()}
                      placeholder={`${priceForMealType(form.meal_type_id)}`}
                    />
                  </FieldLabel>
